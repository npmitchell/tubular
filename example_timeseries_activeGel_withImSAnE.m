...
% cd /Volumes/WOON/tubular_test/activeGel/202203221330_region3_ATayarGel/timesequence/ ;
...
if ~exist('./masterSettings.mat', 'file') || true
    ...
    timePoints = 0:29;       % timepoints to include in the analysis
    ...
    fn = 'reg3_T%02d';        % filename string pattern
...
if exist(msls_detOpts_fn, 'file') && false
    ...
    detectOptions = struct( ...
        ...
        'run_full_dataset', 'none', ...projectDir,... % projectDir, ... % set to 'none' for single tp
        'radius_guess', 60, ...
        'dset_name', 'exported_data',...
        'save', true, ... % whether to save images of debugging output
        'center_guess', '50,50,10',... % xyz of the initial guess sphere ;
        'plot_mesh3d', false, ...
        'dtype', 'h5',...
        'mask', 'none',...
        ...
        'smooth_with_matlab', true, ... % set this to >0 to use matlab laplacian filte
        'pythonVersion', '' );  
    
    % VERSION WITH 'integralDetector'
    % detectOptions = struct( ...
    %     'channel', 1, ...
    %     'ssfactor', 4, ...
    %     'niter', 35,...
