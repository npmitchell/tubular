...
if ~exist(fullfile(dataDir, 'xp.mat'), 'file')
    ...
    if exist(detOpts_fn, 'file')
        ...
        ilastikaxisorder= 'xyzc'; ... % axis order as output by ilastik probabilities h5
        ...
        detectOptions = struct( 'channel', 1, ...
            ...
            'center_guess', 'click',... % xyz of the initial guess sphere ;
...
idOptions.ssr_thres = 1500 ;  % distance of sum squared residuals in um as threshold for removing spurious centerlines
...
% Note: since the tube converts coiling into twist, we start with timepoint
% 10 as our t0 to get the cutpath to have the right winding number around
% the centerline.
t0_for_phi0= 10 ;  % Do this "reference" timepoint first.
tubi.t0set(t0_for_phi0)
write_txt_with_header(tubi.fileName.t0, tubi.t0, '#reference timepoint')
...
%     % Smoothing the function phi(u) is helpful if there is a lot of twist
%     % in the tubular surface.
...
% Note that here we choose width = 0 so there is effectively no smoothing
options = struct() ;
