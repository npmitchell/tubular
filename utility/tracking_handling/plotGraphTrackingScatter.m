function plotGraphTrackingScatter(G, MIPFile, saveDir, Options)
% plot timeseries of tracking results stored as digraph as scatterplot
% with dots colored by track/Segment/Generation on top of MIPFile
%
% Parameters
% ----------
% G : digraph
...
%       Include visual information on Generation # in plot
%   plotVoronoi : bool
%       Include Voronoi tesselation in image
%   faceAlpha : float (0-1)
...
if isfield(Options, 'plotVoronoi')
    plotVoronoi = Options.plotVoronoi ;
end
...
if isfield(Options, 'sz')
    sz = Options.sz ;
elseif isfield(Options, 'markerSize')
...
for tidx =  1:numel(timePoints)
    ...
    scatter(U(:, 1), U(:, 2), sz, fSegColors, 'filled')
