...
[v0n, v0t, facenormals] = resolveTangentNormalVector(mesh.f, mesh.v, vf) ;
...
[V2F, F2V] = meshAveragingOperators(F, V3D);
...
%% Compare to Discrete Exterior Calculus (DEC) ============================
clc;

%--------------------------------------------------------------------------
% Construct the Divergence of the Vector Field
%--------------------------------------------------------------------------

% Calculate the divergence using the DEC ----------------------------------

DEC = DiscreteExteriorCalculus(F, V3D);
divDEC = DEC.divergence(UF);

% divDEC = V2F * divDEC;

% Calculate the divergence using the covariant derivative -----------------

% Construct the fundamental forms of the surface interms of the pullback
[g, b] = constructFundamentalForms(F, V3D, V2D);
% g = inducedMetric(F, V3D, V2D);

divCov = zeros(size(F,1), 1);
for i = 1:size(F,1)
    divCov(i) = trace( inv(g{i}) * (DU2D{i} + DU2D{i}.') ./ 2 );
    % divCov(i) = trace( inv(testG{i}) * (CDU{i} + CDU{i}.') ./ 2 );
end

% Average the face-based derivative onto vertices
divCov = F2V * divCov;

% Calculate the analytic divergence ---------------------------------------

if numel(divCov) == size(V3D,1)
    
    x = V2D(:,1); y = V2D(:,2);
    
elseif numel(divCov) == size(F,1)
    
    x = COM2D(:,1); y = COM2D(:,2);

end

divTru = (3 - 3 .* x.^6 - 29 .* y.^2 + 29 .* y.^4 - 3 .* y.^6 + x.^4 .* ...
    (29 - 9 .* y.^2) + x.^2 .* (-29 + 58 .* y.^2 - 9 .* y.^4)) ./ ...
    (2 .* (1 + x.^2 + y.^2).^3);

divTru(abs(divTru) < 1e-14) = 0;

%--------------------------------------------------------------------------
% Calculate Errors
%--------------------------------------------------------------------------

divErr = abs(divCov - divDEC) ./ abs(divDEC);
% divErr = abs(divCov - divTru) ./ abs(divTru);
% divErr = abs(divDEC - divTru);


divErr( isinf(divErr) | isinf(divErr) ) = 0;
...
if numel(divErr) == size(V3D,1)
    
trisurf( sphereTri, 'FaceColor', 'interp', ...
    'FaceVertexCData', divErr, 'EdgeColor', 'none' );

elseif numel(divErr) == size(F,1)
    
    trisurf( sphereTri, 'FaceColor', 'flat', ...
    'FaceVertexCData', divErr, 'EdgeColor', 'none' );
...
%% ************************************************************************
% *************************************************************************
%     PART FOUR: THE COVARIANT DERIVATIVE ON TOPOLOGICAL ANNULI
% *************************************************************************
% *************************************************************************
clear; close all; clc;

%--------------------------------------------------------------------------
% Create Cylinder Cut Mesh
%--------------------------------------------------------------------------

% Create basic grid points in 2D
L = 5; % The length of the cylinder in 3D
NL = 100; % The number of vertices along the length of the cylinder
NW = round( 2 * pi * NL ./ L); % The number of vertices along the width
[X, Y] = meshgrid( linspace(0, L, NL), linspace(0, 2*pi, NW) );
...
V3D = [ R .* cos(V2D(:,2)), R .* sin(V2D(:,2)), V2D(:,1) ];

cylTri = triangulation(F, V3D);

% Re-scale the 2D vertices if desired
% V2D = [ V2D(:,1), V2D ./ max(V2D(:,2)) ];
...
% trisurf(triangulation(F, V3D)); axis equal
% triplot(triangulation(F, V2D)); axis equal

%--------------------------------------------------------------------------
% Construct Cylindrical Surface Tangent Vectors
%--------------------------------------------------------------------------

s = V2D(:,1);
phi = V2D(:,2);

eSV = [ zeros(size(V3D,1), 2), ones(size(V3D,1), 1) ];
ePV = R .* [ -sin(phi), cos(phi), zeros(size(V3D,1), 1) ];
...
% plotVec = ePV;
% ssf = 1;
% trisurf(cylTri, 'FaceColor', [0.8 0.8 0.8], 'EdgeColor', 'k' );
% hold on
% quiver3( V3D(1:ssf:end, 1), V3D(1:ssf:end, 2), V3D(1:ssf:end, 3), ...
%     plotVec(1:ssf:end,1), plotVec(1:ssf:end,2), plotVec(1:ssf:end,3), ...
%     1, 'LineWidth', 2, 'Color', 'm' );
...
UV = repmat(VSV, 1, 3) .* eSV + repmat(VPV, 1, 3) .* ePV;

% View results ------------------------------------------------------------
% ssf = 2; % Sub-sampling factor for visualization
% trisurf( cylTri, 'FaceColor', [0.8 0.8 0.8], 'EdgeColor', 'none' );
% hold on
% quiver3( V3D(1:ssf:end, 1), V3D(1:ssf:end, 2), V3D(1:ssf:end, 3), ...
%     UV(1:ssf:end, 1), UV(1:ssf:end, 2), UV(1:ssf:end, 3), ...
%     1, 'LineWidth', 2, 'Color', 'm' );
...
% trisurf( cylTri, 'FaceColor', [0.8 0.8 0.8], 'EdgeColor', 'none' );
% hold on 
% quiver3( COM3D(1:ssf:end,1), COM3D(1:ssf:end,2), COM3D(1:ssf:end,3), ...
%     UF(1:ssf:end, 1), UF(1:ssf:end, 2), UF(1:ssf:end, 3), ... 1,
%     'LineWidth', 2, 'Color', 'm' );
% hold off
% axis equal
...
CDUF = zeros(size(F,1), 4);
s = COM2D(:,1); phi = COM2D(:,2);

CDUF(:,1) = ones(size(CDUF(:,1)));

CDUF(:,3) = R.^2 .* sin(phi);

CDUF(:,4) = R.^2 .* s .* cos(phi);
...
% View Results
%--------------------------------------------------------------------------
trisurf( cylTri, 'FaceColor', 'flat', 'FaceVertexCData', covErr, ...
    'EdgeColor', 'none' );
axis equal
colorbar
set(gca, 'Clim', [0 1]);
...
% divErr = abs(divCov - divDEC) ./ abs(divDEC);
divErr = abs(divDEC - divTru) ./ abs(divTru);
% divErr = abs(divDEC - divTru);
...
if numel(divErr) == size(V3D,1)
    ...
    trisurf( cylTri, 'FaceColor', 'interp', ...
        'FaceVertexCData', divErr, 'EdgeColor', 'none' );
    
    % patch( 'Faces', F, 'Vertices', V2D, 'FaceColor', 'interp', ...
    %     'FaceVertexCData', divErr, 'EdgeColor', 'none' );
    
elseif numel(divErr) == size(F,1)
    
    trisurf( cylTri, 'FaceColor', 'flat', ...
        'FaceVertexCData', divErr, 'EdgeColor', 'none' );
    
    % patch( 'Faces', F, 'Vertices', V2D, 'FaceColor', 'flat', ...
    %     'FaceVertexCData', divErr, 'EdgeColor', 'none' );
