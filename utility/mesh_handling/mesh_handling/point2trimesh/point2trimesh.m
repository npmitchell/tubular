function [ distances, surface_points, closest_faces, ...
    faces2, vertices2, ...
    corresponding_vertices_ID, new_faces_ID ] = point2trimesh( varargin )
% -------------------------------------------------------
%
...
%      - nearest_faces  (#qPoints   x 1)   Vector with the nearest face to each query point, added by NPMitchell
...
%
%
% NPMitchell: added nearest face returned as output and progress report
...
switch parser.Results.Algorithm
    ...
    case 'vectorized'
        ...
        P = P'; % (#qPoints x xyz)
        if nargout > 2
            disp('WARNING: Have not coded for returning nearby faces in vectorized mode')
        end
...
closest_faces = F ;  % (#qPoints x 1 int)
