...
function [xyz,val,iter,serie] = wFRnd6_wgravity(tol,xyz,NL,BL,KL,TRISm,BLm,KLm,mg,nd,projectfcn,varargin)

serie = [];

%NL(find(NL==0))=1;

[F,dF] = wFree_Energy4_dF_wgravity(xyz,NL,BL,KL,TRISm,BLm,KLm,mg);
...
for i=1:3*size(xyz,1)
    ...
    eps = fminbnd(@wFmin4_wgravity,0,1,[],xyz,H,TRISm,BLm,KLm,mg,projectfcn,varargin{:});
    %eps;
