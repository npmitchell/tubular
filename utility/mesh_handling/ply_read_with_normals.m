function [ Elements, varargout ] = ply_read_with_normals ( Path, Str )
...
%
%   [TRI,PTS,NORMALS,QUALITY,DATA,COMMENTS] = PLY_READ(FILENAME,'tri') 
%   converts vertex and face data into triangular connectivity and vertex 
%   arrays.  The mesh can then be displayed using the TRISURF command.
%
...
%    
%    Noah Mitchell added functionality to load normals February 2019. As of
%    now, second argument 'tri' may be required to properly load normals.
%
%  Licensing:
%
...
%  Author:
%
%    Pascal Getreuer 2004, modified by Noah Mitchell 2019
%
%  Parameters:
%
%  Local Parameters:
...
%
  ...
  if ( ( nargin > 1 && strcmpi(Str,'Tri') ) || nargout > 2 )
    ...
    % 
    % Also look for normal vectors
    %
    Names
    if ( any(strcmp(Names,'nx')) && any(strcmp(Names,'ny')) && any(strcmp(Names,'nz')) )
      eval(['varargout{2}=[Elements.',Name,'.nx,Elements.',Name,'.ny,Elements.',Name,'.nz];']);
    else
      vargout{2} = zeros(1, 3);  
    end

    % 
    % Also look for a quality field (scalar field at each vertex)
    %
    if ( any(strcmp(Names,'quality')) )
      eval(['varargout{3}=[Elements.',Name,'.quality];']);
    else
      vargout{3} = zeros(1);  
    end
    varargout{4} = Elements;
    varargout{5} = Comments;
    Elements = [];
    ...
    if ( ~isempty(Names) )
      ...
      if ( ~iscell(PropertyName) )
        ...
        eval(['FaceIndices=varargout{4}.',Name,'.',PropertyName,';']);
