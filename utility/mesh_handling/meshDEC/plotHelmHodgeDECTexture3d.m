function plotHelmHodgeDECTexture3d(IV, cutMesh, divs, rots, rot, trans, Options)
...
for divcurl = 1:2 
    ...
    tMapOpts.PSize = 5 ;
    % tMapOpts.Rotation = rot ;
    % tMapOpts.Translation = trans ;
    tMapOpts.ApplyAmbientOcclusion = false ;
    tMapOpts.AmbientOcclusionFactor = 0.3 ;
    tMapOpts.AmbientOcclusionSamples = 1000 ;
    tMapOpts.AddLights = true ;
    tMapOpts.ScalarField = sfs{divcurl} ;
    tMapOpts.ScalarCLim = [-sscales(divcurl), sscales(divcurl)] ;
    tMapOpts.ScalarColorMap = bwr ;
    if isfield(cutMesh, 'vn')
        tMapOpts.VertexNormals = cutMesh.vn ;
    end
    
    % Texture image
    texture_patch_3d_heatmap2file(FF, v3d, FF, v3drs, IV, tMapOpts, opts)
