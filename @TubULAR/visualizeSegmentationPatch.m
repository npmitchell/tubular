...
%       timepoints to visualize, must have segmentation for these
%       timepoints
%   t0 : int 
...
if isfield(Options, 'demoPatchName')
    ...
    for qq = 1:length(outDirs)
        if exist(fullfile(outDirs(qq).folder, outDirs(qq).name), 'dir')
            nDirsThatExistAleady = nDirsThatExistAleady + 1 ;
        end
    end
    dpname = sprintf('demoPatch%03d', nDirsThatExistAleady+1) ;
end
...
for tii = 1:length(timePoints)
    ...
    outSegFn = fullfile(outFixedDir, sprintf(['seg_' dpname '_%06d.pdf'], tp)) ;  
    outImRawFn = fullfile(outFixedDir, sprintf(['raw_' dpname '_%06d.png'], tp)) ;  

    if ~exist(outFigFn, 'file') || ~exist(outImFn, 'file') || ...
            ~exist(outSegFn, 'file') || overwrite || true
        % Load pullback
        if strcmpi(coordSys, 'spsme')
        ...
        if tii == 1
            ...
            if ~exist('Fsz', 'var')
                outFigFn0 = fullfile(outFixedDir, ...
                    sprintf([dpname '_%06d.png'], timePoints(1))) ;
        ...
        if tii == 1
            ...
            if ~exist('Asz', 'var')
                outImFn0 = fullfile(outFixedDir, ...
                    sprintf(['im_' dpname '_%06d.png'], timePoints(1))) ;
        ...
        % Save image only, for manipulation of LUT etc in post with overlay
        imwrite(imRGB_c, outImRawFn) ;
        
        % Save segmentation only, for manipulation of LUT etc in post
        clf
        pp = polygonsFromSegmentation(seg.segIm(yminIm:ymaxIm, xminIm:xmaxIm)) ;
        % imshow(segRGB_c)
        for cellID = 1:numel(pp)
            if ~isempty(pp{cellID})
                hold on;            
                patch(pp{cellID}(:, 2), pp{cellID}(:, 1), ...
                    colors(cellID, :), ...
...
for tii = tidx2do
    ...
    if ~exist(segImOutFn, 'file') || ~exist(outdatFn, 'file') || ...
        ...
        % Save segmentation only, for adjustments of LUT etc in post
        clf
        for cellID = 1:Ncells
            hold on;
            patch(segV2D{cellID}(:, 1), segV2D{cellID}(:, 2), ...
