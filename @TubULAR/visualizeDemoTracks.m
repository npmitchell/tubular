function visualizeDemoTracks(QS, Options) 
    ...
    if ~floatingFrame
        for qq = 1:length(tracks2demo)
            ...
            colors = get_colors_for_demoTracks(Ncells) ;
...
function colors = get_colors_for_demoTracks(Ncells)
    % colors = distinguishable_colors(Ncells, [0,0,0;1,1,1], brighten) ;
    colors = ones(Ncells, 3) ;
    hues = linspace(0, 1, Ncells+1) ;
    colors(:, 1) = hues(1:end-1) ;
