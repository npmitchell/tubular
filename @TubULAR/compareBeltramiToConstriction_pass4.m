...
if recompute        
    ...
    meshScaled.u = m4scale.u ;
    meshScaled.v = m4scale.vrs ;
    ...
    for tidx = tidx2do
        ...
        %% RECOMPUTE MU
        this_mu = bc_metric(meshScaled.f, meshScaled.u, mesh.v, 3) ;
        this_mu = F2V * this_mu ;
        % Mode filter mu
        filterOptions.widthX = 3 ;
        filterOptions.nmodesY = 5 ;
        filterOptions.preview = false ;
        % truncate final u row for mode filtering
        this_mu = reshape(this_mu, [nU, nV-1]) ;
        this_mu_re = modeFilterQuasi1D(real(this_mu), filterOptions) ;
        this_mu_im = modeFilterQuasi1D(imag(this_mu), filterOptions) ;
        this_mu = this_mu_re(:) + 1j * this_mu_im(:) ;
        this_mu(nU*(nV-1)+1:nU*nV) = this_mu(1:nU) ;
        
        
        diff_mu = mu_pred_vtx - reshape(this_mu, size(mu_pred_vtx)) ;
        ...
        [axs, cbs, meshHandles] = nFieldsOnSurface(mesh, ...
            {real(mu_pred_vtx), ...
            real(squeeze(this_mu)), ...
            real(diff_mu), ...
            imag(mu_pred_vtx), ...
            imag(squeeze(this_mu)), ...
            imag(diff_mu)}, ...
            plotOpts) ;
