function visualizeMeshEvolution(tubi, options)
...
forcetrue = true ;
growtht0 = tubi.xp.fileMeta.timePoints(1) ;
lwX = 2 ;
...
%% Directories
if plot_growth
    outdirGPLY = fullfile(tubi.dir.mesh, 'demo_morphsnakes_figs', ...
        sprintf('growth_plys_%06dt0', growtht0)) ;
    outdirGF = fullfile(tubi.dir.mesh, 'demo_morphsnakes_figs', ...
        sprintf('growth_faceon_%06dt0', growtht0)) ;
    outdirGP = fullfile(tubi.dir.mesh, 'demo_morphsnakes_figs', ...
...
% ----------------------
outdirF = fullfile(tubi.dir.mesh, 'demo_morphsnakes_figs', 'faceon') ;
outdirP = fullfile(tubi.dir.mesh, 'demo_morphsnakes_figs', 'perspective') ;
...
if plot_growth
    ...
    dtype = '.h5' ;
    mlxprogram = tubi.xp.detectOptions(1).mlxprogram ;
    ...
    for tt = -1:(nFiles-1)
        ...
        if tt >= 0
            ...
            fnGP = fullfile(outdirGP, 'mesh', [sprintf(tubi.fileBase.name, tt) '.png']) ;
            fnGF = fullfile(outdirGF, 'mesh', [sprintf(tubi.fileBase.name, tt) '.png']) ;
            fn_textureGP = fullfile(outdirGP, 'texture', [sprintf(tubi.fileBase.name, tt) '.png']) ;
            fn_textureGF = fullfile(outdirGF, 'texture', [sprintf(tubi.fileBase.name, tt) '.png']) ;
            use_mesh = true ;
        else                
        ...
        if ~exist(fn_textureGP, 'file') ||  ~exist(fn_textureGF, 'file') || overwrite
            ...
            titlestr = ['$t = $' num2str(tp*tubi.timeInterval-t0) ' ' tubi.timeUnits] ;
...
if plot_meshOnly
    projectDir = tubi.dir.data ;
    meshDir = tubi.dir.mesh ;
    [rot, trans] = tubi.getRotTrans ;
    resolution = tubi.APDV.resolution ;
    plyfn = tubi.fullFileBase.mesh ;
    ...
    for ii = tidx2do 
        ...
        fn_meshP = fullfile(outdirP, 'mesh_only', [sprintf(tubi.fileBase.name, tp) '.png']) ;
        fn_meshF = fullfile(outdirF, 'mesh_only', [sprintf(tubi.fileBase.name, tp) '.png']) ;
        if ~exist(fn_meshP, 'file') || overwrite
            
            tubi.setTime(tp) ;

            % Raw and APDV aligned Meshes
            amesh = tubi.loadCurrentAlignedMesh() ;
...
if plot_evolution
    outdirF = fullfile(tubi.dir.mesh, 'demo_morphsnakes_figs', 'faceon') ;
    outdirP = fullfile(tubi.dir.mesh, 'demo_morphsnakes_figs', 'perspective') ;
    outdirX = fullfile(tubi.dir.mesh, 'demo_morphsnakes_figs', 'outline') ;
    dirs2do = {fullfile(outdirP, 'mesh_only'), ...
    ...
    first = true ;
    tidx2do = [tubi.xp.tIdx(tubi.t0set())-30:30:tubi.xp.tIdx(length(timePoints))] ;
    ...
    for ii = tidx2do
        ...
        fnP = fullfile(outdirP, 'mesh', [sprintf(tubi.fileBase.name, tp) '.png']) ;
        fnF = fullfile(outdirF, 'mesh', [sprintf(tubi.fileBase.name, tp) '.png']) ;
        fnX = fullfile(outdirX, [sprintf(tubi.fileBase.name, tp) '.png']) ;
        fn_textureP = fullfile(outdirP, 'texture', [sprintf(tubi.fileBase.name, tp) '.png']) ;
        fn_textureF = fullfile(outdirF, 'texture', [sprintf(tubi.fileBase.name, tp) '.png']) ;
        fn_texture2P = fullfile(outdirP, 'texture_orthoviews', [sprintf(tubi.fileBase.name, tp) '.png']) ;
        fn_texture2F = fullfile(outdirF, 'texture_orthoviews', [sprintf(tubi.fileBase.name, tp) '.png']) ;
        ...
        if ~exist(fnP, 'file') || ~exist(fn_textureP, 'file') || ...
            ...
            if flipy 
                axyzshift = amesh.v - (tubi.normalShift * resolution) * amesh.vn ;
            else
                axyzshift = amesh.v + (tubi.normalShift * resolution) * amesh.vn ;
            end
        ...
        if ~exist(fnX, 'file') || overwrite 
            ...
            amesh_raw = tubi.loadCurrentAlignedMesh() ;
            amesh_raw.v = amesh_raw.v - ...
               0.5 * tubi.normalShift * amesh_raw.vn * tubi.APDV.resolution ;
        ...
        if plot_texture && (~exist(fn_textureP, 'file') ||  ~exist(fn_textureF, 'file') || overwrite)
            ...
            rmesh = tubi.loadCurrentRawMesh() ;          
            rmesh.v = rmesh.v + tubi.normalShift * rmesh.vn ;
            xr = rmesh.v(:, 1) ;
            yr = rmesh.v(:, 2) ;
            zr = rmesh.v(:, 3) ;
