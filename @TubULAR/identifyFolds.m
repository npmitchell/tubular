function identifyFolds(tubi, options)
%IDENTIFYFOLDS(tubi, OPTIONS)
%   Identify fold locations & lobes in the spcutMesh of tubi.
%
% Parameters
% ----------
...
%   Note that fold_onset is in units of tubi.xp.fileMeta.timepoints, NOT 
...
uvexten = tubi.uvexten ;  % string like sprintf('_nU%04d_nV%04d', nU, nV) ;
...
% overwrite tubi preview option if present in local options
...
if isfield(options, 'meshCoords')
    switch lower(options.meshCoords)
        case 'sp'
            spcutMeshBase = tubi.fullFileBase.spcutMesh ;
        case 'sp_sm'
            spcutMeshBase = tubi.fullFileBase.spcutMeshSm ;
        otherwise 
...
foldfn = tubi.fileName.features.folds ;
if exist(foldfn, 'file') && ~overwrite
    ...
    readme = struct(...
        'timeInterval', 'float, dt for timestamps in tubi.timeUnits', ...
